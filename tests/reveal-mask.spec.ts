 ...
 * mask's bottom padding is load-bearing in two directions: too shallow and the Fraunces-italic
...
test("hero reveal mask clears the italic descenders without unsealing the word", async ({ page }) => {
  ...
  const mask = await page.evaluate(() => {
    ...
    return { glyphs: [ink("g"), ink("y"), ink("h"), ink("Every")], maskDepth, travel: inner.getBoundingClientRect().height * 1.3 };
  });
